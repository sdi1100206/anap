...
void Grid :: pick_t(double rand_t){
	/*
	const float MIN_RAND = 0.0, MAX_RAND = this->delta;
	const float range = MAX_RAND - MIN_RAND;
	float random = range * ((((float) rand()) / (float) RAND_MAX)) + MIN_RAND ;
	this->t = random ;
	this->grid_measurement = this->t + this->delta ; 
	*/
...
vector< vector<double> > Grid :: grid_a_curve( vector <double> Non_grided_curve){
	...
	//		cout<<grids_y << endl;
		...
		//cout <<  "distancex :" <<distancex << "distancey :" << distancey ;
